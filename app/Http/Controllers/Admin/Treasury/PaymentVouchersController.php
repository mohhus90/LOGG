...
use App\Models\{TreasuryVoucher, CashBox, BankAccount, Customer, Supplier, Employee, ChartOfAccount, PurchaseInvoice};
...
class PaymentVouchersController extends Controller
{
    ...
    {
        $query = TreasuryVoucher::where('com_code', $this->comCode())->where('voucher_type', 'payment');
        ...
        return view('admin.treasury.payments.index', compact('data'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.treasury.payments.create', compact('cashBoxes', 'banks', 'customers', 'suppliers', 'employees', 'accounts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $voucher = TreasuryService::createVoucher($this->comCode(), 'payment', $request->all(), Auth::guard('admin')->id());
        } catch (\RuntimeException $e) {
            return back()->withInput()->with('error', $e->getMessage());
        }

        if ($request->filled('purchase_invoice_id')) {
            $invoice = PurchaseInvoice::where('com_code', $this->comCode())->find($request->purchase_invoice_id);
            if ($invoice) {
        ...
        return redirect()->route('treasury_payments.show', $voucher->id)->with('success', 'تم تسجيل سند الصرف بنجاح');
    }

    public function show($id)
    {
        $voucher = TreasuryVoucher::with(['cashBox', 'bankAccount', 'cheque', 'createdBy'])
            ->where('com_code', $this->comCode())->where('voucher_type', 'payment')->findOrFail($id);
        return view('admin.treasury.payments.show', compact('voucher'));
    }
}
