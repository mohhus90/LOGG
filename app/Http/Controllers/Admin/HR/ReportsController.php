...
class ReportsController extends Controller
{
    ...
    // ─────────────────────────────────────────────
    //  تنبيهات انتهاء العقود وفترة الاختبار
    // ─────────────────────────────────────────────
    public function contractsExpiring(Request $request)
    {
        ...
        $probationEmployees = Employee::where('com_code', $this->comCode())
            ->whereNotNull('probation_end_date')
            ->whereBetween('probation_end_date', [$today, $until])
            ->orderBy('probation_end_date')
            ->get();

        $contractEmployees = Employee::where('com_code', $this->comCode())
            ->whereNotNull('contract_end_date')
            ->whereBetween('contract_end_date', [$today, $until])
            ->orderBy('contract_end_date')
            ->get();

        return view('admin.reports.contracts_expiring', compact('probationEmployees', 'contractEmployees', 'days'));
    }
