...
@section('title') تعديل فئة أصول @endsection
...
<div class="col-lg-7 col-md-9">
    <div class="card card-primary card-outline">
        <div class="card-header"><h3 class="card-title"><i class="fas fa-edit ml-2"></i> تعديل فئة أصول</h3></div>
        <form action="{{ route('asset_categories.update', $category->id) }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="default_useful_life_years" class="form-control" value="{{ old('default_useful_life_years', $category->default_useful_life_years) }}" min="1" required>
                </div>
                <div class="form-group">
                    ...
                    <select name="asset_gl_account_id" class="form-control select2" required>
                        ...
                        @foreach($accounts as $acc)
                            <option value="{{ $acc->id }}" {{ $category->asset_gl_account_id == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="accum_depreciation_gl_account_id" class="form-control select2" required>
                        <option value="">-- اختر الحساب --</option>
                        @foreach($accounts as $acc)
                            <option value="{{ $acc->id }}" {{ $category->accum_depreciation_gl_account_id == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="depreciation_expense_gl_account_id" class="form-control select2" required>
                        <option value="">-- اختر الحساب --</option>
                        @foreach($accounts as $acc)
                            <option value="{{ $acc->id }}" {{ $category->depreciation_expense_gl_account_id == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
                        @endforeach
                    </select>
                </div>
