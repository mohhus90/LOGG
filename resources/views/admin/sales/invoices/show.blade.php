...
@section('title') فاتورة {{ $invoice->invoice_number }} @endsection
...
<div class="col-12">
    ...
    <div class="mb-3 d-flex justify-content-between align-items-center flex-wrap" style="gap:8px">
        <div>
            <a href="{{ route('sales_invoices.edit', $invoice->id) }}" class="btn btn-warning btn-sm">
                <i class="fas fa-edit ml-1"></i> تعديل
            </a>
            <a href="{{ route('sales_invoices.print', $invoice->id) }}" class="btn btn-secondary btn-sm" target="_blank">
                <i class="fas fa-print ml-1"></i> طباعة
            </a>
            @if($remaining > 0 && ($invoice->status ?? '') !== 'cancelled')
            <a href="{{ route('sales_payments.create', ['invoice_id' => $invoice->id]) }}" class="btn btn-success btn-sm">
        ...
        <div class="d-flex align-items-center" style="gap:8px">
            @switch($payStatus)
                @case('paid')    <span class="badge badge-success p-2" style="font-size:1rem">مسدّد بالكامل</span> @break
                @case('partial') <span class="badge badge-warning p-2" style="font-size:1rem">مسدّد جزئياً</span> @break
                @case('unpaid')  <span class="badge badge-danger  p-2" style="font-size:1rem">غير مسدّد</span> @break
            @endswitch
            @switch($invoice->status ?? 'issued')
                @case('draft')     <span class="badge badge-secondary p-2" style="font-size:1rem">مسودة</span> @break
                @case('issued')    <span class="badge badge-primary   p-2" style="font-size:1rem">مُصدَرة</span> @break
                @case('cancelled') <span class="badge badge-danger    p-2" style="font-size:1rem">ملغاة</span> @break
            @endswitch
        </div>
    </div>
            ...
            <div class="card card-outline card-primary">
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        <tr><th style="width:130px" class="text-muted">العميل</th><td><strong>{{ $invoice->customer->name ?? '—' }}</strong></td></tr>
                        <tr><th class="text-muted">الهاتف</th><td>{{ $invoice->customer->phone ?? '—' }}</td></tr>
                        <tr><th class="text-muted">البريد</th><td>{{ $invoice->customer->email ?? '—' }}</td></tr>
                        <tr><th class="text-muted">الرقم الضريبي</th><td>{{ $invoice->customer->tax_number ?? '—' }}</td></tr>
                        <tr><th class="text-muted">العنوان</th><td>{{ $invoice->customer->address ?? '—' }}</td></tr>
                    </table>
                </div>
            </div>
        </div>
            ...
            <div class="card card-outline card-warning">
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        ...
                        @if($invoice->order_id)
                        <tr><th class="text-muted">أمر البيع</th><td><a href="{{ route('sales_orders.show', $invoice->order_id) }}">{{ $invoice->order->order_number ?? $invoice->order_id }}</a></td></tr>
                        @endif
                    </table>
                </div>
            </div>
        ...
        <div class="card-body p-0">
            <table class="table table-bordered table-hover mb-0">
                ...
                <tfoot class="bg-light">
                    <tr>
                        <td colspan="4" class="text-left font-weight-bold">إجمالي المدفوع:</td>
            ...
            <div class="card card-outline card-primary">
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        ...
                        <tr>
                            <td class="text-success"><strong>المحصّل:</strong></td>
