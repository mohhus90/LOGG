...
@section('title') تقرير ملخص المبيعات @endsection
...
<div class="col-12">
    ...
    <div class="card card-outline card-secondary mb-3">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('sales_reports.summary') }}">
                <div class="row">
                    <div class="col-md-4">
                ...
                @if(request()->hasAny(['from','to']))
                    <a href="{{ route('sales_reports.summary') }}" class="btn btn-sm btn-secondary">
    ...
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">
                <i class="fas fa-file-alt ml-2"></i> ملخص فواتير المبيعات
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('sales_invoices.show', $invoice->id) }}" class="text-primary">
                            ...
                            <td class="text-success">{{ number_format($invoice->paid ?? 0, 2) }}</td>
                            <td class="{{ ($invoice->remaining ?? 0) > 0 ? 'text-danger font-weight-bold' : 'text-success' }}">
                                {{ number_format($invoice->remaining ?? 0, 2) }}
                            </td>
                            <td>
                                @php $remaining = $invoice->remaining ?? 0; $total = $invoice->total ?? 0; @endphp
                                @if($remaining <= 0)
                                    <span class="badge badge-success">مسدّد</span>
                                @elseif($remaining < $total)
                                    <span class="badge badge-warning text-dark">جزئي</span>
                                @else
                                    <span class="badge badge-danger">غير مسدّد</span>
                                @endif
                            </td>
                        </tr>
                        @empty
