...
@section('title') تعديل أمر بيع @endsection
...
@section('content')
<div class="col-12">
    <form action="{{ route('sales_orders.update', $order->id) }}" method="POST" id="orderForm">
        @csrf

        <div class="card card-warning card-outline">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-edit ml-2"></i> تعديل أمر البيع رقم: <strong>{{ $order->order_number }}</strong>
                </h3>
            </div>
            <div class="card-body">
                @if($errors->any())
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" class="form-control"
                                   value="{{ old('date', \Carbon\Carbon::parse($order->date)->format('Y-m-d')) }}" required>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="date" name="delivery_date" class="form-control"
                                   value="{{ old('delivery_date', $order->delivery_date ? \Carbon\Carbon::parse($order->delivery_date)->format('Y-m-d') : '') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="number" name="tax_rate" id="taxRate" class="form-control"
                                   value="{{ old('tax_rate', $order->tax_rate ?? 14) }}" min="0" max="100" step="0.01">
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                ...
                                <option value="processing" {{ old('status', $order->status) == 'processing' ? 'selected' : '' }}>قيد التنفيذ</option>
                                <option value="partial"    {{ old('status', $order->status) == 'partial'    ? 'selected' : '' }}>تسليم جزئي</option>
                                <option value="delivered"  {{ old('status', $order->status) == 'delivered'  ? 'selected' : '' }}>مُسلَّم</option>
        ...
        <div class="card card-warning card-outline">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-list ml-2"></i> بنود أمر البيع</h3>
            ...
            <div class="col-md-5">
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body totals-box">
                        ...
                        <div class="d-flex justify-content-between mb-2 align-items-center">
                            ...
                            <div class="d-flex align-items-center">
                                <select name="discount_type" id="discountType" class="form-control form-control-sm ml-1" style="width:80px">
                                    <option value="percent" {{ old('discount_type', $order->discount_type) == 'percent' ? 'selected' : '' }}>%</option>
                                    <option value="amount"  {{ old('discount_type', $order->discount_type) == 'amount'  ? 'selected' : '' }}>مبلغ</option>
                                </select>
                                <input type="number" name="discount_value" id="discountValue"
                                       class="form-control form-control-sm" style="width:80px"
                                       value="{{ old('discount_value', $order->discount_value ?? 0) }}" min="0" step="0.01">
                            </div>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                        ...
                        <input type="hidden" name="subtotal"        id="subtotal"       value="{{ $order->subtotal ?? 0 }}">
                        <input type="hidden" name="discount_amount" id="discountAmount" value="{{ $order->discount_amount ?? 0 }}">
                        <input type="hidden" name="tax_amount"      id="taxAmount"      value="{{ $order->tax_amount ?? 0 }}">
                        <input type="hidden" name="total"           id="total"          value="{{ $order->total ?? 0 }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-footer bg-white border-top-0 px-0">
            ...
            <a href="{{ route('sales_orders.show', $order->id) }}" class="btn btn-secondary mr-2">
...
<script>var existingItems = @json($order->items ?? []);</script>
@endsection
