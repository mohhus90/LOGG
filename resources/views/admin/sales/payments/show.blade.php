...
<div class="col-12">
    <div class="card card-info card-outline">
        <div class="card-header">
            <h3 class="card-title">
                ...
                @switch($payment->payment_method)
                    @case('cash')
                        <span class="badge badge-success mr-2"><i class="fas fa-money-bill-alt ml-1"></i>نقدي</span>
                        @break
                    @case('bank')
                        <span class="badge badge-primary mr-2"><i class="fas fa-university ml-1"></i>تحويل بنكي</span>
                        @break
                    @case('cheque')
                        <span class="badge badge-warning text-dark mr-2"><i class="fas fa-money-check ml-1"></i>شيك</span>
                        @break
                    @case('pos')
                        <span class="badge badge-info mr-2"><i class="fas fa-credit-card ml-1"></i>بطاقة ائتمان</span>
                        @break
                @endswitch
            </h3>
        ...
        <div class="card-body">
            ...
            <div class="row">
                            ...
                            <table class="table table-bordered mb-0">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @switch($payment->payment_method)
                                            @case('cash')
                                                <span class="badge badge-success badge-lg"><i class="fas fa-money-bill-alt ml-1"></i>نقدي</span>
                                                @break
                                            @case('bank')
                                                <span class="badge badge-primary badge-lg"><i class="fas fa-university ml-1"></i>تحويل بنكي</span>
                                                @break
                                            @case('cheque')
                                                <span class="badge badge-warning text-dark badge-lg"><i class="fas fa-money-check ml-1"></i>شيك</span>
                                                @break
                                            @case('pos')
                                                <span class="badge badge-info badge-lg"><i class="fas fa-credit-card ml-1"></i>بطاقة ائتمان</span>
                                                @break
                                        @endswitch
                                    </td>
                                </tr>
                ...
                <div class="col-md-6">
                    <div class="card card-light">
                        <div class="card-header bg-light">
                            <h5 class="card-title mb-0"><i class="fas fa-user ml-2 text-primary"></i>معلومات العميل والفاتورة</h5>
                        </div>
                        <div class="card-body p-0">
                            <table class="table table-bordered mb-0">
                                <tr>
                                    <th class="bg-light" width="40%">العميل</th>
                                    <td>
                                        @if($payment->customer)
                                            <strong>{{ $payment->customer->name }}</strong>
                                            @if($payment->customer->phone)
                                                <br><small class="text-muted"><i class="fas fa-phone ml-1"></i>{{ $payment->customer->phone }}</small>
                                            @endif
                                        @else
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($payment->invoice)
                                            <a href="{{ route('sales_invoices.show', $payment->invoice_id) }}" class="text-primary">
        ...
        <div class="card-footer">
            <a href="{{ route('sales_payments.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-right ml-1"></i> رجوع للقائمة
            </a>
            <a href="{{ route('sales_payments.delete', $payment->id) }}" class="btn btn-danger mr-2"
