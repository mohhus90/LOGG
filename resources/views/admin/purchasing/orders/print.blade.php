...
<head>
    ...
    <title>أمر شراء رقم {{ $order->order_number }}</title>
    <style>
...
<div class="print-page">
    ...
    <div class="info-grid">
        <div class="info-box">
            <h4>بيانات أمر الشراء</h4>
            <table>
                <tr><td>رقم الأمر:</td><td>{{ $order->order_number }}</td></tr>
                <tr><td>التاريخ:</td><td>{{ \Carbon\Carbon::parse($order->date)->format('Y/m/d') }}</td></tr>
                <tr><td>تاريخ الاستلام:</td><td>{{ $order->expected_date ? \Carbon\Carbon::parse($order->expected_date)->format('Y/m/d') : '—' }}</td></tr>
                <tr>
                    <td>الحالة:</td>
                    <td>
                        @switch($order->status)
                            ...
                            @case('partial')   <span class="status-badge" style="background:#ffc107;color:#333">استلام جزئي</span> @break
                            @case('received')  <span class="status-badge" style="background:#28a745">مستلم</span> @break
        ...
        <div class="info-box">
            <h4>بيانات المورد</h4>
            <table>
                <tr><td>الاسم:</td><td>{{ $order->supplier->name ?? '—' }}</td></tr>
                <tr><td>الهاتف:</td><td>{{ $order->supplier->phone ?? '—' }}</td></tr>
                <tr><td>البريد:</td><td>{{ $order->supplier->email ?? '—' }}</td></tr>
                <tr><td>الرقم الضريبي:</td><td>{{ $order->supplier->tax_number ?? '—' }}</td></tr>
                <tr><td>العنوان:</td><td>{{ $order->supplier->address ?? '—' }}</td></tr>
            </table>
        </div>
    </div>
