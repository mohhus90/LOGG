...
@section('title') أمر شراء {{ $order->order_number }} @endsection
...
<div class="col-12">
    ...
    <div class="mb-3 d-flex justify-content-between align-items-center flex-wrap" style="gap:8px">
        <div>
            <a href="{{ route('purchase_orders.edit', $order->id) }}" class="btn btn-warning btn-sm">
                <i class="fas fa-edit ml-1"></i> تعديل
            </a>
            <a href="{{ route('purchase_orders.print', $order->id) }}" class="btn btn-secondary btn-sm" target="_blank">
                <i class="fas fa-print ml-1"></i> طباعة
            </a>
            @if(in_array($order->status, ['confirmed', 'partial']))
            <form action="{{ route('purchase_orders.invoice', $order->id) }}" method="POST" class="d-inline"
            ...
            <div class="btn-group">
                ...
                <div class="dropdown-menu">
                    @foreach(['confirmed' => 'مؤكد', 'partial' => 'استلام جزئي', 'received' => 'مستلم', 'cancelled' => 'ملغي'] as $val => $label)
                        @if($order->status !== $val)
                        <form action="{{ route('purchase_orders.status', $order->id) }}" method="POST">
                            @csrf
    ...
    <div class="row">
            ...
            <div class="card card-outline card-primary">
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        <tr><th style="width:130px" class="text-muted">المورد</th><td><strong>{{ $order->supplier->name ?? '—' }}</strong></td></tr>
                        <tr><th class="text-muted">الهاتف</th><td>{{ $order->supplier->phone ?? '—' }}</td></tr>
                        <tr><th class="text-muted">البريد</th><td>{{ $order->supplier->email ?? '—' }}</td></tr>
                        <tr><th class="text-muted">الرقم الضريبي</th><td>{{ $order->supplier->tax_number ?? '—' }}</td></tr>
                    </table>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="card card-outline card-warning">
                <div class="card-header"><h3 class="card-title"><i class="fas fa-file-signature ml-2"></i> بيانات أمر الشراء</h3></div>
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        ...
                        <tr><th class="text-muted">تاريخ الاستلام المتوقع</th><td>{{ $order->expected_date ? \Carbon\Carbon::parse($order->expected_date)->format('Y/m/d') : '—' }}</td></tr>
                        <tr><th class="text-muted">الفرع</th><td>{{ $order->branch->name ?? '—' }}</td></tr>
                        @if($order->request_id)
                        <tr><th class="text-muted">مرجع طلب الشراء</th><td><a href="{{ route('purchase_requests.show', $order->request_id) }}">{{ $order->request->request_number ?? $order->request_id }}</a></td></tr>
                        @endif
                    </table>
                </div>
            </div>
    ...
    <div class="card card-outline card-secondary">
        <div class="card-header"><h3 class="card-title"><i class="fas fa-list ml-2"></i> بنود أمر الشراء</h3></div>
                ...
                <table class="table table-bordered table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            $pct = $item->quantity > 0 ? round($received / $item->quantity * 100) : 0;
                        @endphp
                        <tr>
                            <td>{{ $i + 1 }}</td>
            ...
            <table class="table table-bordered table-sm mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('purchase_invoices.show', $invoice->id) }}" class="btn btn-xs btn-info">
