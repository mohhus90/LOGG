...
@section('title') تعديل فاتورة شراء @endsection
...
@section('content')
<div class="col-12">
    <form action="{{ route('purchase_invoices.update', $invoice->id) }}" method="POST" id="invoiceForm">
        @csrf

        <div class="card card-warning card-outline">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-edit ml-2"></i> تعديل فاتورة الشراء رقم: <strong>{{ $invoice->invoice_number }}</strong></h3>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="invoice_type" class="form-control">
                                <option value="cash"   {{ old('invoice_type', $invoice->invoice_type) == 'cash'   ? 'selected' : '' }}>نقدي</option>
                            ...
                            <select name="branch_id" class="form-control select2">
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ old('branch_id', $invoice->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" name="supplier_invoice_no" class="form-control" value="{{ old('supplier_invoice_no', $invoice->supplier_invoice_no) }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label>نسبة الضريبة %</label>
                            <input type="number" name="tax_rate" id="taxRate" class="form-control" value="{{ old('tax_rate', $invoice->tax_rate ?? 14) }}" min="0" max="100" step="0.01">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body totals-box">
                        ...
                        <div class="d-flex justify-content-between mb-2 align-items-center">
                            ...
                            <input type="number" name="discount_amount" id="discountAmountInput"
                                   ...
                                   value="{{ old('discount_amount', $invoice->discount_amount ?? 0) }}" min="0" step="0.01">
                        </div>
                        <div class="d-flex justify-content-between mb-2">
        ...
        <div class="card-footer bg-white border-top-0 px-0">
            ...
            <a href="{{ route('purchase_invoices.show', $invoice->id) }}" class="btn btn-secondary mr-2">
