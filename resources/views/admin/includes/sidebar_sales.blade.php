<div class="sidebar">
  ...
  <nav class="mt-2">
    <ul class="nav nav-pills nav-sidebar flex-column nav-sidebar-rtl"
      ...
      {{-- POS --}}
      @php $posOpen = request()->is('admin/dashboard/sales/pos*'); @endphp
      <li class="nav-item has-treeview {{ $posOpen ? 'menu-open' : '' }}">
        <a href="#" class="nav-link {{ $posOpen ? 'active' : '' }}">
          <i class="nav-icon fas fa-cash-register"></i>
          <p>نقطة البيع (POS) <i class="right fas fa-angle-left"></i></p>
        </a>
        <ul class="nav nav-treeview">
          <li class="nav-item">
            <a href="{{ route('pos.select_register') }}"
               class="nav-link {{ request()->routeIs('pos.select_register') || request()->routeIs('pos.terminal') ? 'active' : '' }}">
              <i class="nav-icon fas fa-cash-register"></i><p>شاشة الكاشير</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="{{ route('pos_sessions.index') }}"
               class="nav-link {{ request()->is('admin/dashboard/sales/pos-sessions*') ? 'active' : '' }}">
              <i class="nav-icon fas fa-receipt"></i><p>جلسات الكاشير</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="{{ route('pos_registers.index') }}"
               class="nav-link {{ request()->is('admin/dashboard/sales/pos-registers*') ? 'active' : '' }}">
              <i class="nav-icon fas fa-store"></i><p>ماكينات الكاشير</p>
            </a>
          </li>
        </ul>
      </li>
